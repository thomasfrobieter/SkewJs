...
function Skew(elements, settings) {
  ...
  else if (elements.nodeType) this.targets = [elements];
  else this.targets = elements;
    ...
    if (self.options.breakpoints !== undefined) {
      ...
      self.options.breakpoints.forEach(function (breakpoint) {
        if (breakpoint.break >= winWidth) {
          if (breakpoint.debounce !== undefined) debounce = breakpoint.debounce;
...
Skew.prototype = {
  skew: function () {
    ...
    if (self.options.breakpoints !== undefined) {
      ...
      self.options.breakpoints.forEach(function (breakpoint) {
        if (breakpoint.break < winWidth) return;

        if (breakpoint.x !== undefined) skewX = breakpoint.x;
        if (breakpoint.y !== undefined) skewY = breakpoint.y;
      ...
      if (self.currentUnskewContent !== unskewContent) {
        ...
        for (var i = 0; i < self.targets.length; i++) {
          ...
          if (self.currentUnskewContent === true) content = target.childNodes;
          else if (
            typeof self.currentUnskewContent == "string" &&
            self.currentUnskewContent != ""
          )
    ...
    for (var i = 0; i < self.targets.length; i++) {
      ...
      if (unskewContent === true) content = target.childNodes;
  ...
  destroy: function () {
    ...
    for (var i = 0; i < self.targets.length; i++) {
      ...
      var skewEndIndex =
        skewIndex + style.substring(skewIndex).indexOf(")") + 1;
  ...
  parseSettings: function (settings, defaults) {
    ...
    var addEventListener = function (eventArray, listeners) {
      if (listeners !== undefined)
        ...
        else eventArray.push(listeners);
    };

    if (settings !== undefined) {
      if (settings.x !== undefined) results.x = settings.x;

      if (settings.y !== undefined) results.y = settings.y;
      ...
      if (Array.isArray(settings.breakpoints)) {
        ...
        settings.breakpoints.forEach(function (breakpoint) {
          ...
          if (breakpoint.x !== undefined) breakObj.x = breakpoint.x;

          if (breakpoint.y !== undefined) breakObj.y = breakpoint.y;
        ...
        results.breakpoints.sort(function (a, b) {
          if (a.break < b.break) return 1;
          if (a.break > b.break) return -1;
          return 0;
        });
      }

      if (settings.debounce !== undefined) results.debounce = settings.debounce;
      ...
      addEventListener(
        results.beforeElementSkewListeners,
        settings.beforeElementSkew
      );

      results.afterElementSkewListeners = defaults.afterElementSkewListeners;
      addEventListener(
        results.afterElementSkewListeners,
        settings.afterElementSkew
      );
    }
    return results;
  },
  ...
  setSkewStyle: function (element, skewStyle) {
    ...
    var transformEndIndex =
      transformIndex + style.substring(transformIndex).indexOf(";");
    ...
    transformEndIndex =
      transformIndex + style.substring(transformIndex).indexOf(")") + 1;
